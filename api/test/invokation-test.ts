...
import { EntryPoint } from '@app-composer/types';
import { InvocationApi } from '../src/functions/invocation-api';
...
describe('Invokation', () => {
  ...
  test('write invokation', () => {
    const invocation = new InvocationApi('foo');
    ...
    assert.equal(
      data.indexOf(`${projectName} = require(`) >= 0,
      true,
      `'projectName' not in generated code`
    );
    assert.equal(
      data.indexOf('appServer.addInvokable') >= 0,
      true,
      `'addInvokeable' not in generated code`
    );
    assert.equal(
      data.indexOf('8080') >= 0,
      true,
      `expected default port 8080 in generated code`
    );
  });
});
