...
export function extractor(
  ...
  const promiseNames = names.map(
    u =>
      new Promise<Names>((rs, rj) => {
        ...
        mkdirp(tmpDir, undefined, err => {
          ...
          execa('sh', ['-c', `cd ${tmpDir} && tar xzf ${u.npmPackage}`])
            .then(() => {
              const packageDir = path.join(tmpDir, 'package');
              u.packageJson = JSON.parse(
                fs
                  .readFileSync(path.join(packageDir, 'package.json'))
                  .toString()
              );
              const ijp = `${path.join(
                path.dirname(u.npmPackage),
                path.basename(u.npmPackage, '.npm.tgz')
              )}.Invocation.json`;
              try {
